...
import { BACKEND_URL_A } from "./assets/Config"; // Import the fetch URL

export default function FormA372() {
  const [currentTime, setCurrentTime] = useState(new Date().toLocaleString());

  const navigate = useNavigate();
  const navigateUrl = "/372/status_a";

  // Form states
  const [dataset, setDataset] = useState("CIFAR-10");
  const [model, setModel] = useState("VGG16");
  const [strategy, setStrategy] = useState("DSTA");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`http://${BACKEND_URL_A}/start`, {
        dataset,
        model,
        strategy,
        epochs,
        ...
        <Box sx={{ marginTop: "0px", padding: "12px" }}>
          ...
          >
            <Paper elevation={3} sx={{ p: 4, borderRadius: "16px" }}>
              ...
              <FormControl fullWidth sx={{ mb: 3 }}>
                <InputLabel>设备选择策略</InputLabel>
                <Select
                  value={strategy}
                  label="设备选择策略"
                  onChange={(e) => setStrategy(e.target.value)}
                >
                  <MenuItem value="DSTA">DSTA</MenuItem>
                  <MenuItem value="Oort">Oort</MenuItem>
                  <MenuItem value="FedAvg">FedAvg</MenuItem>
                  <MenuItem value="FedCS">FedCS</MenuItem>
                </Select>
              </FormControl>
