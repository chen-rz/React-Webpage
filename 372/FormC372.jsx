...
import { BACKEND_URL_C } from "./assets/Config"; // Import the fetch URL

export default function FormC372() {
  const [currentTime, setCurrentTime] = useState(new Date().toLocaleString());

  const navigate = useNavigate();
  const navigateUrl = "/372/status_c";

  // Form states
  const [dataset, setDataset] = useState("MNIST");
  const [model, setModel] = useState("AlexNet");
  const [strategy, setStrategy] = useState("COCS");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`http://${BACKEND_URL_C}/start`, {
        dataset,
        model,
        strategy,
        numNodes,
          ...
          >
            <Paper elevation={3} sx={{ p: 4, borderRadius: "16px" }}>
              ...
              <FormControl fullWidth sx={{ mb: 3 }}>
                ...
                >
                  <MenuItem value="COCS">COCS</MenuItem>
                  <MenuItem value="Oracle">Oracle</MenuItem>
                  <MenuItem value="COVG">COVG</MenuItem>
                  <MenuItem value="Random">Random</MenuItem>
                  <MenuItem value="CUCB">CUCB</MenuItem>
                </Select>
              </FormControl>
